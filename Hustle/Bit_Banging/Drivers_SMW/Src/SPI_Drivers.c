 ...
 */
void SPI_Byte_Send_Rec(SPI_Handler_t * pSPI_Handler,uint8_t * MSG_send , uint8_t * MSG_REC)
{	uint8_t rec=0x00,send;
	...
	if(pSPI_Handler->SPI_Config.DATA_Format == SPI_DATA_FORMAT_MSBF)
	{
		for(int i=0;i<8;++i)
		{
			rec &= SPI_Bit_Send_Rec(pSPI_Handler, (send & (1<<7)));        //has form of 0b0000000x
			send = send << 1;
			rec = rec << 1;
		}
	}
 ...
 */
void SPI_MSG_Send_Rec(SPI_Handler_t * pSPI_Handler, uint8_t * MSG_send , uint8_t * MSG_REC, uint32_t Length )
{
	if(pSPI_Handler->SPI_Config.STATUS == SET)
	{
		...
		{
			SPI_Byte_Send_Rec(pSPI_Handler, MSG_send, MSG_REC);
			MSG_send++;
			MSG_REC++;
		}
